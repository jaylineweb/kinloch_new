 ...
 */
 ...
 (function($) {
     ...
     $(function() {
         ...
         $lnbDepthItem.on('mouseenter.menu focusin.menu', function(event) { //mouseover
             if(mode === 'pc') {
                 var $this = $(this),
             ...
             if(mode === 'mobile') {
                 var $this = $(this),
                     ...
                     //$depth4Item = $(this.hasClass('.depth4_item')),
                 ...
                 if($depthText.find(eventTarget).length || $depthText[0]  === eventTarget) {
                     ...
                     */
                     ...
                     if ($('body').hasClass('www')) {
                         if ($this.children('.depth5')) {
                             $this.find('.depth5').removeClass('depth');
                             $this.find('.depth6').removeClass('depth');
                         }
                     } else {
                         if ($this.children('.depth4')) {
                             $this.find('.depth4').removeClass('depth');
                             $this.find('.depth5').removeClass('depth');
                             $this.find('.depth6').removeClass('depth');
                         }
                     }
                     ...
                     $("a.depth_text.depth2_text.spy[data-menu-key='3517']").click(function(){
                             location.href="https://www.songpa.go.kr/hanseong/contents.do?key=3517&";
                     });
                     $("a.depth_text.depth2_text.spy[data-menu-key='3517'] + div.depth.depth3").css({"visibility":"hidden"});
                     /*if($this.children('.depth').legnth){
                         if($this.hasClass('.depth4_item')){
                     ...
                     /*else if($this.children('.depth').length(){
                         if($this.hasClass("depth4_text")){
                             event.target();
                         }
                     }*/
                     /*if(){
         ...
         $lnbMenu.on('click.menu', function(event) { //mouseleave
             if(mode === 'pc') {
                 $lnbMenu.height('');
                 $html.removeClass('lnb_open');
                 $lnbDepthItem.removeClass('active');
         ...
         $window.on('screen:tablet.menu screen:phone.menu', function(event) {
             ...
             if($lnbSpy.length) {
                 //$html.addClass('lnb_open'); 2020.08.24 신명화 주석처리
                 $lnbSpy.parents('.depth_item').addClass('active');
             }
         });
 
         // $(window).on("load",function(){
         //     if($(".lnb_show.lnb_open .lnb .menu .depth1_item").hasClass(""){
